...
import { Canvas, FabricImage, Rect, Point } from 'fabric';
...
export function ImageCropper({ image, onCrop, onCancel }: ImageCropperProps) {
  ...
  useEffect(() => {
    ...
    reader.onload = (e) => {
      ...
      FabricImage.fromURL(url).then((img) => {
        ...
        // 이미지를 배경 뒤가 아니라 오버레이 아래에 배치해야 함
        // canvas.add(img, topRect, bottomRect, leftRect, rightRect, border);
        // 레이어 순서: 이미지 -> 오버레이
        canvas.add(img);
  ...
  const handleDone = async () => {
    ...
    // 하이라이트된 영역만 캡처하기 위해 임시 캔버스 사용하거나 toDataURL의 crop 옵션 지적
    // toDataURL에서 left, top, width, height 지정 가능
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-black flex flex-col animate-in fade-in">
      ...
      <div className="p-6 bg-black/80 backdrop-blur-md flex items-center justify-around">
      ...
      <div className="bg-black p-4 pb-8 text-center">
